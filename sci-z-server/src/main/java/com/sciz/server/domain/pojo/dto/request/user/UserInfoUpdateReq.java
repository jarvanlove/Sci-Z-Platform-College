 ...
 * @param smsCode    String 手机验证码（修改手机号时必填）
 *
 * @author JiaWen.Wu
...
public record UserInfoUpdateReq(
        @NotBlank(message = "用户名不能为空") @Size(min = 3, max = 20, message = "用户名长度为3-20个字符") @Pattern(regexp = "^[a-zA-Z0-9_]+$", message = "用户名只能包含字母、数字和下划线") String username,
