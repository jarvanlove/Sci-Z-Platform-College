package com.sciz.server.infrastructure.shared.event.file;

import com.sciz.server.infrastructure.shared.event.DomainEvent;
import lombok.Getter;
import lombok.Setter;

/**
 * 文件删除事件
 *
 * @author JiaWen.Wu
 * @className FileDeletedEvent
 * @date 2025-10-29 11:30
 */
@Getter
@Setter
public class FileDeletedEvent extends DomainEvent {

    private String fileId;
    private String fileName;
    private String fileType;
    private String filePath;
    private String bucketName;
    private String userId;
    private String userName;
    private String projectId;
    private String projectName;
    private String deleteTime;
    private String deleteReason;

    public FileDeletedEvent(String fileId, String fileName, String fileType,
            String filePath, String bucketName, String userId,
            String userName, String projectId, String projectName,
            String deleteTime, String deleteReason) {
        super();
        this.fileId = fileId;
        this.fileName = fileName;
        this.fileType = fileType;
        this.filePath = filePath;
        this.bucketName = bucketName;
        this.userId = userId;
        this.userName = userName;
        this.projectId = projectId;
        this.projectName = projectName;
        this.deleteTime = deleteTime;
        this.deleteReason = deleteReason;
    }

    @Override
    public String getAggregateId() {
        return fileId;
    }

    @Override
    public String getAggregateType() {
        return "File";
    }
}
