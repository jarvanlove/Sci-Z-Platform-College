...
public class ProxyController {
    ...
    public Result<PdfCacheResp> cachePdf(@RequestParam String url, @RequestParam String id) {
        try {
            PdfCacheService.PdfCacheResult result = pdfCacheService.downloadAndCache(url, id);
    ...
    @Operation(summary = "获取缓存的PDF文件", description = "根据业务文件id获取PDF流。先查Redis(id→文件路径)：有则从MinIO取文件返回；无则需传url，下载后上传MinIO并缓存。超时20秒。")
    @GetMapping("/pdf")
    public void getCachedPdf(@RequestParam String id,
                              @RequestParam(required = false) String url,
                              HttpServletResponse response) {
        InputStream inputStream = null;
        try {
            if (id == null || id.isEmpty()) {
                writeErrorResponse(response, HttpStatus.BAD_REQUEST.value(), "文件id不能为空", false);
                return;
            }
            
            // 使用业务文件id直接获取或创建PDF
            PdfCacheService.PdfStreamResult streamResult;
            try {
                CompletableFuture<PdfCacheService.PdfStreamResult> future = CompletableFuture.supplyAsync(() -> {
                    try {
                        // id作为业务文件id，直接传入
                        return pdfCacheService.getOrCreatePdfByFileId(id, url);
                    } catch (BusinessException e) {
                        throw new RuntimeException(e);
                    }
                });
                streamResult = future.get(20, TimeUnit.SECONDS);
            } catch (TimeoutException e) {
                log.warn("获取PDF文件超时（20秒）: id={}", id);
                ...
                if (cause instanceof BusinessException) {
                    ...
                    log.warn("获取缓存的PDF文件失败: id={}, err={}", id, be.getMessage());
                ...
                writeErrorResponse(response, HttpStatus.INTERNAL_SERVER_ERROR.value(),
                        "获取PDF文件失败: " + (cause != null ? cause.getMessage() : e.getMessage()), false);
                return;
            }
            
            inputStream = streamResult.stream();
            String fileName = streamResult.fileName();
            ...
            log.info("返回缓存的PDF文件: id={}, fileName={}", id, fileName);
