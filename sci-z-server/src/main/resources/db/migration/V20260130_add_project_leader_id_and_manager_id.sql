-- 申报表增加项目负责人用户ID（用于权限：指定负责人与创建人等价权限）
ALTER TABLE declaration ADD COLUMN IF NOT EXISTS project_leader_id BIGINT;
COMMENT ON COLUMN declaration.project_leader_id IS '项目负责人用户ID';

-- 项目表增加项目负责人用户ID（与申报同步，用于权限判断：创建人+当前负责人均有负责人权限）
ALTER TABLE project ADD COLUMN IF NOT EXISTS manager_id BIGINT;
COMMENT ON COLUMN project.manager_id IS '项目负责人用户ID';
